...
class State:
    ...
    def distToSphere(self, sphere):
        return np.linalg.norm(self.ps[-1] - sphere[0:3]) - R - sphere[-1]
...
def targetRRT(tree, goalstate, Nmax):
    ...
    while True:
        ...
        if nearstate.ConnectsTo(nextstate) and np.all(nearstate.ts != nextstate.ts):
            ...
            # if  nextstate.Distance(goalstate) <= MAX_STEP and nextstate.ConnectsTo(goalstate):
...
def plan():
    ...
    print('Running with step size ', MAX_STEP, ' and up to ', Nmax, ' nodes.')
